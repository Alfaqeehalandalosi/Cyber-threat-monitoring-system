...
def render_threats_table(data: Dict[str, Any]):
    """Render threats table with sorting and search"""
    ...
    def style_threat_score(val):
        color_class = get_threat_score_color(val)
        return f'<span class="threat-score-badge {color_class}">{val:.2f}</span>'
    
    styled_df = df.copy()
    styled_df['Threat Score'] = styled_df['Threat Score'].apply(style_threat_score)
    
    st.dataframe(
        styled_df[['Title', 'Threat Score', 'Primary Threat', 'Source']],
...
def main():
    ...
    <div class="refresh-info">
        <span style="color: #00ccff;">⏰ Auto-refresh in {int(time_until_refresh)} seconds</span>
    </div>
    """, unsafe_allow_html=True)
